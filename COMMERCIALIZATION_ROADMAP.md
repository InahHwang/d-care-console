...
**해결 방안**:
- [x] 사용자 가입/로그인 시 clinicId를 JWT에 포함
- [x] 모든 DB 쿼리에 `clinicId` 필터 추가 (55개 V2 라우트)
- [x] 기존 데이터 마이그레이션 API (`/api/v2/migrate/clinic-id`)
- [ ] 치과(클리닉) 관리 컬렉션 생성 (`clinics`) — 2번째 고객 시
- [ ] 데이터 격리 검증 테스트
...
**해결 방안**:
- [x] 테스트 계정 코드 제거
- [x] CORS 허용 도메인 제한 (환경변수로 관리)
- [x] 보안 헤더 추가 (X-Frame-Options, X-Content-Type-Options 등)
...
| 1 | 보안 기본 조치 | ✅ 완료 | 2026-02-21 |
| 1 | 멀티테넌시 | ✅ 완료 | 2026-02-22 |
| 1 | 입력값 검증 | ✅ 완료 | 2026-02-22 |
| 2 | Sentry 연동 | ✅ 완료 | 2026-02-22 |
| 2 | Error Boundary | ✅ 완료 | 2026-02-22 |
