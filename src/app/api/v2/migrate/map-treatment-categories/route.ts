...
export async function POST(request: NextRequest) {
  try {
    ...
    let mappedCount = 0;
    const addedUncategorized: string[] = [];
    ...
    // 2. 매핑된 대분류별 "미분류" 시스템 항목 추가
    const parentCategories: string[] = Array.from(new Set(
      updatedTypes
        .filter((t: any) => t.parentCategory)
        .map((t: any) => t.parentCategory as string)
    )) as string[];

    for (let i = 0; i < parentCategories.length; i++) {
      const pc = parentCategories[i];
      const uncategorizedId = `uncategorized_${pc.replace(/\s+/g, '_')}`;
      const exists = updatedTypes.some((t: any) => t.id === uncategorizedId);
    ...
    return NextResponse.json({
      ...
      mappedCount,
      addedUncategorized,
      totalTreatmentTypes: updatedTypes.length,
    });
  } catch (error) {
