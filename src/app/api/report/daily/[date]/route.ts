// src/app/api/report/daily/[date]/route.ts
// 일별 마감 리포트 API (PC/모바일 공용)
...
export async function GET(
  ...
  try {
    ...
    console.log(`[DailyReport] ${selectedDate} 일별 마감 리포트 조회`);
    ...
    console.log(`[DailyReport] ${selectedDate} 관련 환자 수: ${dailyPatients.length}명`);
...
export async function POST(request: NextRequest) {
  try {
    ...
    // PC용, 모바일용 URL 모두 생성
    const pcUrl = `${baseUrl}/report/daily/${date}?token=${reportToken}`;
    const mobileUrl = `${baseUrl}/report/mobile/${date}?token=${reportToken}`;
    ...
    console.error('[DailyReport] 토큰 생성 오류:', error);
    return NextResponse.json(
