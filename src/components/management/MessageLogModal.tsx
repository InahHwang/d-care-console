...
export default function MessageLogModal({ isOpen, onClose, patientId, embedded = false }: MessageLogModalProps) {
  ...
  // 로컬 상태
  const [filterVisible, setFilterVisible] = useState(embedded ? true : false);
  ...
  // 컴포넌트 마운트 시 MongoDB에서 로그 로드
  useEffect(() => {
    if (isOpen) {
      // localStorage에서 임시 데이터 먼저 표시 (빠른 초기 로딩)
      dispatch(initializeLogs());
      // MongoDB에서 실제 데이터 가져오기
      dispatch(fetchMessageLogs());
      dispatch(fetchCategories());

      // 특정 환자의 로그만 표시
      if (patientId) {
        dispatch(setFilters({ patientId }));
      }
    }
