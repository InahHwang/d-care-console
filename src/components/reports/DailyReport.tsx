...
const DailyReport: React.FC = () => {
  ...
  const fetchReport = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/report/daily/${selectedDate}`, {
        headers: {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-gray-900">일별마감보고서</h2>
          <p className="text-sm text-gray-600 mt-1">
            {data?.clinicName} - {data?.date} ({data?.dayOfWeek})
          </p>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
      ...
      {data && data.patients.length === 0 && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
          <div className="text-6xl mb-4">📋</div>
          <h3 className="text-lg font-semibold text-gray-900 mb-2">상담 내역이 없습니다</h3>
          <p className="text-gray-600">{selectedDate}에 등록된 상담 내역이 없습니다.</p>
        </div>
      )}
    </div>
  );
